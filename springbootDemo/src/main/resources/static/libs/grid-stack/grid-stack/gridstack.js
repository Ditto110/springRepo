...
(function (factory) {
    ...
    var GridStack = function (el, opts) {
        ...
        this._defineColumWidth(self.opts.width);
    };
    ...
    GridStack.prototype._defineColumWidth = function (width) {
        if (this._styles === null || typeof this._styles === 'undefined') {
            return;
        }
        var prefix = '.' + this.opts._class + ' .' + this.opts.itemClass;
        var size = (100 / width).toFixed(5);
        for (var i = 0; i < width; i++) {
            Utils.insertCSSRule(this._styles,
                prefix + '[data-gs-width="' + (i + 1) + '"]',
                'width: ' + size * (i + 1) + '%;',
                i
            );
            Utils.insertCSSRule(this._styles,
    ...
    GridStack.prototype._prepareElementsByNode = function (el, node) {
        ...
        var dragOrResize = function (event, ui) {
            ...
            if (node._grid.opts.resizeCallback != undefined && typeof node._grid.opts.resizeCallback == "function") {
                if (ui.element != undefined) {
                    node._grid.opts.resizeCallback(ui.element, node.width, node.height);
                }
            }
            if (node._grid.opts.dragOrResizeCallback != undefined && typeof node._grid.opts.dragOrResizeCallback == "function") {
                node._grid.opts.dragOrResizeCallback();
            }
            self._updateContainerHeight();
        };
    ...
    GridStack.prototype._prepareElement = function (el, triggerAddEvent) {
        ...
        var node = self.grid.addNode({
            ...
            // 源码修改
            widthPX: el.attr('data-gs-widthPX'),
            heightPX: el.attr('data-gs-heightPX'),
            indexOfPanel: el.attr('data-gs-indexOfPanel'),
            temp: el.attr('data-gs-temp'),
    ...
    GridStack.prototype.addWidget = function (el, node) {
        el = $(el);
        ...
        if (typeof node.autoPosition != 'undefined') {
            el.attr('data-gs-auto-position', node.autoPosition ? 'yes' : null);
        }
        if (typeof node.minWidth != 'undefined') {
        ...
        if (typeof node.image != 'undefined' && node.image != '') {
            el.attr('data-gs-image', node.image);
        }
        // TODO 源码修改
        // 像素值 展示
        if (typeof node.widthPX != 'undefined') {
            el.attr('data-gs-widthPX', node.widthPX);
        }
        if (typeof node.heightPX != 'undefined') {
            el.attr('data-gs-heightPX', node.heightPX);
        }
        if (typeof node.indexOfPanel != 'undefined') {
            el.attr('data-gs-indexOfPanel', node.indexOfPanel);
        }
        if (typeof node.temp != 'undefined') {
            el.attr('data-gs-temp', node.temp);
        } else {
            el.attr('data-gs-temp', 0);
        }
        ...
        if (typeof node.locked != 'undefined' && node.locked == 1) {
            this.movable(el, false);
            this.resizable(el, false);
            this.locked(el, true);
        }
        if (typeof node.image != 'undefined' && node.image != "") {
            el.find(".item-content").css("background-image", "url('" + node.image + "')");
        }
        // TODO 源码在这修改了
        if (this.opts.resizeCallback != undefined && typeof this.opts.resizeCallback == "function") {
            this.opts.resizeCallback(el, node.widthPX, node.heightPX);
        }
        return el;
    };
