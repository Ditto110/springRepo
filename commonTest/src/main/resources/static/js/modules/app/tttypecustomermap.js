			...
			{ label: '操作', name: 'opt', width:60,
                formatter: function (value, grid, rows, state) {
                    return '<a class="btn btn-default" onclick="detail('+ rows.id +')">详细</a>'
                        + '&nbsp;&nbsp;'
                        + '<a class="btn btn-default" btnPermission="ttcustomer:update" onclick="edit('+ rows.id +')">配置</a>';
                }
			}
        ],
		viewrecords: true,
...
var vm = new Vue({
	...
	methods: {
        ...
        submit: function () {
		    console.log(vm.customerMaps);

		    var params = {
		      deviceTypeId: vm.deviceTypeIdSelected,
              customers: vm.customerMaps
            };
