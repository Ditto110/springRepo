$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '名称', name: 'name', index: 'name', width: 50 ,formatter: function(cellValue,options,rowObject){
				var	apkUrl = '<a href="'+ rowObject.appVersionEntity.apkcdnurl+'" title="下载应用">'+rowObject.name+'</a>';
	            return apkUrl;
	        } }, 			
			{ label: '图片', name: 'siconpath', index: 'siconpath', width: 40,formatter: function(cellValue,options,rowObject){
				var	imgUrl = '<img src="'+cellValue+'" style="width:50px;height:25px;"/>';
	            return imgUrl;
	        } }, 
	        { label: '一级分类', name: 'classList', index: 'classList', width: 30 ,formatter:function(cellValue,options,rowObject){  
				...
				for(var i in classList){
					if(classList[i].classlevel == 1){
						firstClassName += classList[i].classname;
			...
			{ label: '二级分类', name: 'classList', index: 'classList', width: 30 ,formatter:function(cellValue,options,rowObject){  
				var secondClassNames = "";
				var classList = rowObject.classList;
				for(var i in classList){
					if(classList[i].classlevel == 2){
						secondClassNames += classList[i].classname;
						if(i < classList.length - 1){
							secondClassNames += "\n";
						}
					}
				}
                return secondClassNames;
			}}, 			
			...
			{ label: '蜜币', name: 'beecoin', index: 'beecoin', width: 15 }, 			
			{ label: '维币', name: 'weicoin', index: 'weicoin', width: 15 }, 			
			{ label: '派影视积分', name: 'weiPoint', index: 'weiPoint', width: 30 }, 			
			{ label: '自动更新状态', name: 'autoUpdate', index: 'autoUpdate', width: 30 ,formatter:function(cellValue,options,rowObject){
				if (cellValue == 0) {
					return "打开";
				}else {
					return "关闭";
			...
			{ label: '版本/升级', name: 'id', index: 'id', width: 50,formatter: function(cellValue,options,rowObject){
				var actionHtml = "<a class='btn btn-primary btn-xs' btnPermission='tbapp:version:old' onclick='vm.listOldAppVersion("+rowObject.id+","+rowObject.appVersionEntity.id+")'>旧版本</a>&nbsp;&nbsp;"
							    +"<a class='btn btn-primary btn-xs' btnPermission='tbapp:upgrade' onclick='vm.upgrade("+rowObject.id+")'>升级</a>";
	            return actionHtml;
	        } },
			{ label: '操作', name: 'id', index: 'id', width: 30,formatter: function(cellValue,options,rowObject){
				var actionHtml = "<a class='btn btn-primary btn-xs' btnPermission='tbapp:update' onclick='vm.update("+rowObject.id+");'>修改</a>";
	            return actionHtml;
	        } }		
        ],
		viewrecords: true,
	...
	methods: {
		...
		exportExcel:function(){
			var queryParam = {
					...
					endDate:$("#endDate").val(),
					weiPoint:$("#weiPoint").val()==-1?null:$("#weiPoint").val(),
					autoUpdate:$("#autoUpdate").val()==-1?null:$("#autoUpdate").val()		
				};
			//前端
			var url = baseURL + '/admin/tbapp/exportList';
			var data = "";
			data += "应用ID\t名称\t图片\t一级分类\t二级分类\t最新版本\t文件大小\t更新时间\t蜜币\t维币\t派影视积分\t自动更新状态\t发布状态\t广告插件\t虚拟下载次数\t点赞次数\t";
			data += "\n";
			var fileName ="应用列表";
			$.get(url, queryParam, function(response) {
				...
				for (var i = 0; i < resp.length; i++) {
					data += resp[i].id + "\t";
					data += resp[i].name + "\t";
					data += resp[i].siconpath + "\t";
					var classList =  resp[i].classList;
					var fclass = "";
					var sclass = "";
					for(var j in classList){
						if(classList[j].classlevel == 1){
							fclass += classList[j].classname;
						}else if(classList[j].classlevel == 2){
							sclass += classList[j].classname;
						}
					}
					data += fclass + "\t";
					data += sclass + "\t";
					data += resp[i].appVersionEntity.versionname + "\t";
					data += resp[i].appVersionEntity.apksize + "\t";
					data += resp[i].appVersionEntity.updatetime + "\t";
					data += resp[i].beecoin + "\t";
					data += resp[i].weicoin + "\t";
					data += resp[i].weiPoint + "\t";
					data += (resp[i].autoUpdate==0?"打开":"关闭") + "\t";
					data += (resp[i].appVersionEntity.publish==1?"上架":"下架") + "\t";
					data += (resp[i].appVersionEntity.adflag==1?"有":"无") + "\t";
					data += resp[i].visualdownloadnum + "\t";
					data += resp[i].upnum + "\t";
					data += "\n";
				}
				var BB = self.Blob;
