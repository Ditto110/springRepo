...
function initTable(){
	$("#jqGrid").jqGrid({
        ...
        datatype: "json",
        colNames : ['类别名称', '分类标识码','缩略图',"父分类",'排序','类别操作'],
        colModel: [			
			...
			{  name: 'id', index: 'id', width: 80,formatter: function(value, options, row){
				return [
					"<button type='button' btnPermission='tbclass:update' class='btn btn-info btn-xs' onclick='vm.update("+row.id+")'>修改</button>",
					"<button type='button' btnPermission='tbclass:del' class='btn btn-danger btn-xs' onclick='vm.del("+row.id+")'>删除</button>"
				].join('');
			}}			
        ],
		viewrecords: true,
